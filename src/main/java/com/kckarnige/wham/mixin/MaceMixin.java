                ...
                if (hit.getType() == HitResult.Type.BLOCK) {
                    ...
                    if (Objects.equals(String.valueOf(blockHit.getSide()), "up")) {
                        if (!(player.getStackInHand(hand).getMaxDamage() * 0.70 >= player.getStackInHand(hand).getMaxDamage() - player.getStackInHand(hand).getDamage())) {
                            ...
                            int WindSlamLV = EnchantmentHelper.getLevel(world.getRegistryManager().getWrapperOrThrow(RegistryKeys.ENCHANTMENT).getOrThrow(WhamEnchantment.WIND_CONTROL), player.getStackInHand(hand));
        ...
        if (MidnightConfigStuff.AIR_SLAM) {
            if (!target.isOnGround() && !attacker.isOnGround()) {
                if (attacker instanceof PlayerEntity player) {
                    ...
                    target.damage(target.getWorld().getDamageSources().playerAttack(player), 12f);
                }
            }
        }
    }
